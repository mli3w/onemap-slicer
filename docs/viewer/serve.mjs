...
function proxy(req, res) {
  const upstream = https.request({
    ...
    headers: {
      ...
      // OneMap returns 403 without a matching Referer (checked 2026-05).
      'referer': `https://${UPSTREAM_HOST}/3d/`,
    },
  }, (up) => {
...
http.createServer((req, res) => {
  if (req.url.startsWith('/omapi/') || req.url.startsWith('/maps/') || req.url.startsWith('/api/')) return proxy(req, res);
  serveStatic(req, res);
}).listen(PORT, () => {
